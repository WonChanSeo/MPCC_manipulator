...
def main(args):
    ...
    obs_positions = np.array([
        [0.40, -0.200, 0.521],   # Obstacle 1: Z축으로 이동
    ])

    # 첫 번째 장애물의 이동 한계
    obs_limits = np.zeros((num_obstacles, 2, 3))
    obs_limits[0] = np.array([[0.40, -0.200, 0.421],   # lower limit
                              [0.40, -0.200, 0.621]])  # upper limit
    ...
    if args.name:
        ...
        output_folder = os.path.join("../result/main_w_sim_2", args.name)
