...
: Ts_(Ts),
  ...
  bounds_param_(path.bounds_path),
  integrator_(Ts,path),
...
: Ts_(Ts),
  ...
  bounds_param_(path.bounds_path, param_value.bounds),
  integrator_(Ts,path),
  robot_(new RobotModel()),
  solve_count_(0),
  max_iter_reached_count_(0),
...
void MPC::generateNewInitialGuess_EmergencyStop(const State &x0)
{
    std::cout << "====================================================================" << std::endl;
...
bool MPC::runMPC_(MPCReturn &mpc_return, State &x0, Input &u0, const Eigen::MatrixX3d &obs_positions, const double &obs_radius)
{
    ...
    if(valid_initial_guess_)
        ...
        generateNewInitialGuess_EmergencyStop(x0);    // RTI 모드: 비상정지 기반 initial guess
    ...
    else
    {
        ...
        std::cout << "===================================================" << std::endl;
        valid_initial_guess_ = false;
        num_valid_guess_failed_++;  // MPCC_original과 동일하게 추가
