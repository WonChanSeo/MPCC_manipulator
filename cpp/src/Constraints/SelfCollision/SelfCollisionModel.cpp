...
{
    // ===================================================================
    // =========== ASIC Test Case Logging (Self Collision) ===============
    // ===================================================================
    static int g_self_asic_sample_count = 0;
    static const std::string g_self_asic_output_dir = "/home/mms-wonchan/git/MPCC_manipulator/result/asic_testcases/self_mlp/";
    static bool g_self_asic_dir_initialized = false;

    void init_self_asic_output_dir() {
        if (!g_self_asic_dir_initialized) {
            std::string cmd = "mkdir -p " + g_self_asic_output_dir;
            system(cmd.c_str());
            g_self_asic_dir_initialized = true;
        }
    }

    // FP32를 16진수 비트 표현으로 변환하는 헬퍼 함수
    std::string self_float_to_hex(float value) {
        uint32_t bits;
    ...
    std::pair<Eigen::VectorXd, Eigen::MatrixXd> SelCollNNmodel::calculateMlpOutput(Eigen::VectorXd input, bool time_verbose)
    {
        ...
        for (int layer = 0; layer < mlp_.n_layer; layer++)
        {
            if (layer == 0) // input layer
            {
                ...
                for (int h = 0; h < n_neurons; ++h) {
                    pre_activation_f[0](h) = self_matvec_row_adder_tree(
            ...
            else // hidden layers
            {
                ...
                for (int h = 0; h < n_neurons; ++h) {
                    pre_activation_f[layer](h) = self_matvec_row_adder_tree(
        ...
        // ===== Self Collision MLP 테스트 케이스 저장 =====
        try {
            init_self_asic_output_dir();

            int sample_id = g_self_asic_sample_count++;
            bool is_detailed = (sample_id % 100 == 0);  // 100번째 sample마다 상세 저장

            std::string filename = g_self_asic_output_dir + "sample_" + std::to_string(sample_id) + ".csv";
            std::string filename_bits = g_self_asic_output_dir + "sample_" + std::to_string(sample_id) + "_bits.csv";

            // 첫 번째 샘플일 때만 로그 출력
            if (sample_id == 0) {
                std::cout << "[Self MLP Logging] Starting Self Collision MLP test case logging to: " << g_self_asic_output_dir << std::endl;
            }
            ...
            if (file.is_open() && file_bits.is_open()) {
                ...
                // 헤더 정보
                file << "# Self Collision MLP Test Case - Sample " << sample_id << "\n";
                ...
                // ===== FP32 비트 표현 파일 =====
                file_bits << "# Self Collision MLP Test Case (FP32 Bits) - Sample " << sample_id << "\n";
                ...
                for (int i = 0; i < current_input.size(); ++i) {
                    file_bits << self_float_to_hex(current_input(i));
                ...
                for (int i = 0; i < mlp_.n_output; ++i) {
                    file_bits << self_float_to_hex(output_f(i));
                ...
                for (int r = 0; r < output_derivative_f.rows(); ++r) {
                    for (int c = 0; c < output_derivative_f.cols(); ++c) {
                        file_bits << self_float_to_hex(output_derivative_f(r, c));
                ...
                if (is_detailed) {
                    ...
                    if (mlp_.is_nerf) {
                        ...
                        for (int r = 0; r < nerf_jac_f.rows(); ++r) {
                            for (int c = 0; c < nerf_jac_f.cols(); ++c) {
                                file_bits << self_float_to_hex(nerf_jac_f(r, c));
                    ...
                    for (int layer = 0; layer < mlp_.n_layer - 1; ++layer) {
                        ...
                        for (int h = 0; h < pre_activation_f[layer].size(); ++h) {
                            file_bits << self_float_to_hex(pre_activation_f[layer](h));
                        ...
                        for (int h = 0; h < hidden_f[layer].size(); ++h) {
                            file_bits << self_float_to_hex(hidden_f[layer](h));
                    ...
                    if (mlp_.is_nerf) {
                        ...
                        for (int r = 0; r < jac_rows; ++r) {
                            for (int c = 0; c < jac_cols; ++c) {
                                temp_derivative_log(r, c) = self_matvec_row_adder_tree(
                    ...
                    for (int r = 0; r < temp_derivative_log.rows(); ++r) {
                        for (int c = 0; c < temp_derivative_log.cols(); ++c) {
                            file_bits << self_float_to_hex(temp_derivative_log(r, c));
                    ...
                    for (int layer = 1; layer < mlp_.n_layer - 1; ++layer) {
                        ...
                        for (int r = 0; r < jac_rows; ++r) {
                            for (int c = 0; c < jac_cols; ++c) {
                                new_temp(r, c) = self_matvec_row_adder_tree(
                ...
                if (sample_id == 0) {
                    std::cerr << "[Self MLP Logging ERROR] Failed to open file: " << filename << std::endl;
                }
            }
        } catch (const std::exception& e) {
            std::cerr << "[Self MLP Logging ERROR] Exception during logging: " << e.what() << std::endl;
        }
