...
// Converts the linear system solver enum to a human-readable string. (Updated)
const char* linsys_solver_to_string(enum osqp_linsys_solver_type solver_type) {
  switch (solver_type) {
      case OSQP_DIRECT_SOLVER:   return "Direct";
      case OSQP_INDIRECT_SOLVER: return "Indirect";
      case OSQP_UNKNOWN_SOLVER:
      default:                   return "Unknown";
  }
}

// Converts the CG preconditioner enum to a human-readable string.
const char* cg_precond_to_string(osqp_precond_type precond_type) {
  switch (precond_type) {
      case OSQP_NO_PRECONDITIONER:       return "No Preconditioner";
      case OSQP_DIAGONAL_PRECONDITIONER: return "Diagonal";
      default:                           return "Unknown";
  }
}

// Converts the adaptive rho enum to a human-readable string.
const char* adaptive_rho_to_string(OSQPInt adaptive_rho_type) {
  switch (adaptive_rho_type) {
      case OSQP_ADAPTIVE_RHO_UPDATE_DISABLED:            return "Disabled";
      case OSQP_ADAPTIVE_RHO_UPDATE_ITERATIONS:   return "By Iterations";
      case OSQP_ADAPTIVE_RHO_UPDATE_TIME:         return "By Time";
      case OSQP_ADAPTIVE_RHO_UPDATE_KKT_ERROR:    return "By KKT Error";
      default:                                    return "Unknown";
  }
}
...
/**
 * @brief Prints the OSQP settings to the console.
 *
 * @param settings The settings structure to print.
 */
void osqp_print_settings(const OSQPSettings* s) {
  if (!s) {
      printf("ERROR: OSQPSettings pointer is NULL.\n");
      return;
  }

  printf("--------------------------------------------------\n");
  printf("               OSQP Settings                      \n");
  printf("--------------------------------------------------\n");

  // General and control settings
  printf("linsys_solver: %-20s verbose: %s\n", linsys_solver_to_string(s->linsys_solver), s->verbose ? "On" : "Off");
  printf("scaling: %-23d warm_starting: %s\n", (int)s->scaling, s->warm_starting ? "On" : "Off");
  printf("polishing: %-22s\n", s->polishing ? "On" : "Off");
  printf("allocate_solution: %-15s profiler_level: %d\n", s->allocate_solution ? "On" : "Off", (int)s->profiler_level);
  printf("device: %-24d\n", (int)s->device);
  printf("\n");
  ...
  printf("max_iter: %-21d eps_abs: %.2e\n", (int)s->max_iter, s->eps_abs);
  printf("eps_rel: %-22.2e eps_prim_inf: %.2e\n", s->eps_rel, s->eps_prim_inf);
  printf("eps_dual_inf: %-17.2e scaled_termination: %s\n", s->eps_dual_inf, s->scaled_termination ? "On" : "Off");
  printf("check_termination: %-12d time_limit: %.2e\n", (int)s->check_termination, s->time_limit);
  printf("check_dualgap: %-15d\n", (int)s->check_dualgap);
  printf("\n");

  // Adaptive rho
  printf("adaptive_rho: %-17s adaptive_rho_interval: %d\n", adaptive_rho_to_string(s->adaptive_rho), (int)s->adaptive_rho_interval);
  printf("adaptive_rho_fraction: %.2e   adaptive_rho_tolerance: %.2f\n", s->adaptive_rho_fraction, s->adaptive_rho_tolerance);
  printf("\n");

  // Polishing
  printf("polish_refine_iter: %-11d delta: %.2e\n", (int)s->polish_refine_iter, s->delta);
  printf("\n");
...
# endif /* ifdef OSQP_ENABLE_PROFILING */
  ...
  if (settings->rho_is_vec) {
    ...
    // printf("Setting rho vector...\n");
    set_rho_vec(solver);
  }
  else {
    // printf("Setting rho scalar...\n");
...
#ifdef OSQP_ENABLE_PROFILING // ON
